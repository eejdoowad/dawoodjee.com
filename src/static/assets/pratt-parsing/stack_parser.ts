...
// Code for https://dawoodjee.com/blog/pratt-parsing/ but modified to
// use explicity stacks to pass expression nodes.
//
...
function scan(program: string): Context {
  ...
  return { tokens, index: 0, exprs: [], ops: [] };
}
...
function parse(ctx: Context): Expr {
  ...
  return ctx.exprs.pop()!;
}

function expr(ctx: Context, op: Op): void {
  ctx.ops.push(op);
  expr_head(ctx);
  expr_tail(ctx);
  ctx.ops.pop();
...
function expr_head(ctx: Context): void {
  ...
  if (token === "(") {
    ...
    ctx.exprs.push(number(token));
  } else if (is_prefix_op_token(token)) {
    const op = prefix_op(token);
    expr(ctx, op);
    ctx.exprs.push(prefix(op, ctx.exprs.pop()!));
  } else {
    throw error_bad_token(token ?? "EOF");
  }
}

function expr_tail(ctx: Context): void {
  const parent_op = ctx.ops.at(-1)!;
  while (has_token(ctx)) {
    ...
    if (op === Op.Ternary) {
      ...
      if (peek_token(ctx) === ":") {
        next_token(ctx);
        expr(ctx, op);
        right_expr = ctx.exprs.pop()!;
      }
      const middle_expr = ctx.exprs.pop()!;
      const left_expr = ctx.exprs.pop()!;
      ctx.exprs.push(ternary(left_expr, middle_expr, right_expr));
    } else if (op === Op.Subscript) {
      expr(ctx, Op.Root);
      const rbracket = next_token(ctx);
      if (rbracket !== "]") throw error_bad_token(rbracket, "]");
      const right_expr = ctx.exprs.pop()!;
      const left_expr = ctx.exprs.pop()!;
      ctx.exprs.push(subscript(left_expr, right_expr));
    } else if (is_postfix_op(op)) {
      const right_expr = ctx.exprs.pop()!;
      ctx.exprs.push(postfix(op, right_expr));
    } else if (is_infix_op(op)) {
      expr(ctx, op);
      const right_expr = ctx.exprs.pop()!;
      const left_expr = ctx.exprs.pop()!;
      ctx.exprs.push(infix(op, left_expr, right_expr));
    }
  }
}
...
function assert_parse_error(program: string, expected: string) {
  try {
    ...
    if ((e as Error).message.includes(expected)) return;
    throw new Error(
      `Expected error to contain "${expected}" but got "${
        (e as Error).message
      }"`,
    );
  }
